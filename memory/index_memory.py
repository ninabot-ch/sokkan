...
def parse_note(path: Path) -> dict:
    """Return {name, description, type, priority, body} for a memory note."""
    ...
    if text.startswith("---"):
        ...
        if end != -1:
            ...
            try:
                ...
                # `priority: high` (ou metadata.priority) = fait durable à faire
                # remonter en priorité au recall
                raw_prio = fm.get("priority", meta.get("priority", ""))
                priority = 1 if str(raw_prio).lower() in ("high", "true", "1") else 0
            except yaml.YAMLError:
                pass
    return {"name": name, "description": description, "type": ntype,
            "priority": priority, "body": body.strip()}
...
def init_db(con: sqlite3.Connection) -> None:
    ...
    try:
        con.execute("ALTER TABLE notes ADD COLUMN priority INTEGER DEFAULT 0")
...
def write_memory_index(con: sqlite3.Connection) -> None:
    ...
    """
    rows = con.execute(
        "SELECT name, description, source_path, priority FROM notes "
    ...
    for cap in range(200, 39, -10):
        lines = []
        for name, description, source_path, priority in rows:
            fname = Path(source_path).name
            star = "★ " if priority else ""
            desc = _truncate(" ".join(description.split()), cap)
            lines.append(f"- {star}[{name}]({fname}) — {desc}" if desc else f"- {star}[{name}]({fname})")
        content = header + "\n".join(lines) + "\n"
...
def corpus_signature() -> tuple[int, float]:
    """Cheap change detector: (file count, max mtime) of the notes (MEMORY.md
    excluded — it is regenerated by the indexer itself). Catches edits,
    additions and deletions without opening any file."""
...
def run_index(rebuild: bool = False) -> dict:
    """Index the memory notes into DB_PATH. Reusable in-process (the embedding
    model stays warm in the `embeddings` module between calls); the CLI `main()`
    is a thin wrapper. Returns counters."""
    ...
    try:
        ...
        for path in files:
            ...
            con.execute(
                "INSERT INTO notes(name, description, type, mtime, source_path, priority) "
                "VALUES(?,?,?,?,?,?) ON CONFLICT(name) DO UPDATE SET "
                "description=excluded.description, type=excluded.type, "
                "mtime=excluded.mtime, source_path=excluded.source_path, "
                "priority=excluded.priority",
                (name, note["description"], note["type"], mtime, str(path), note["priority"]),
            )
            con.executemany(
...
def main() -> int:
    ...
    args = ap.parse_args()
    try:
        run_index(rebuild=args.rebuild)
